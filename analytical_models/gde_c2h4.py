...
class System:
    ...
    def set_butler_volmer_coefficients(self):
        ...
        for key in list(self.electrode_reaction_kinetics.keys()):
            if key.startswith('alpha_'):
                self.electrode_reaction_kinetics[key.replace('alpha_', 'b_')] = alpha_to_b(self.electrode_reaction_kinetics[key])
    ...
    def solve(self, phi_ext: Union[float, np.ndarray, torch.Tensor]):
        ...
        k0_CO = A/(2*F) * self.electrode_reaction_kinetics['i_0_CO']/self.chemical_reaction_rates['c_ref'] * self.mod.exp(
            -overpotential_CO/(self.electrode_reaction_kinetics['b_CO']))
        k0_C2H4 = A/(6*F) * self.electrode_reaction_kinetics['i_0_C2H4']/self.chemical_reaction_rates['c_ref'] * self.mod.exp(
            -overpotential_C2H4/(self.electrode_reaction_kinetics['b_C2H4']))
        k0 = k0_CO + k0_C2H4
